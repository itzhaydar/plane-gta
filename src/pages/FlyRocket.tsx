  ...
  useFrame((state, dt) => {
    ...
    const desired = earthMode
      ...
      : marsMode
        ...
        : new THREE.Vector3(6.2, targetY + 5.4, targetZ + 14.5);

    if (!orbit.current?.__dragging) camera.position.lerp(desired, 1 - Math.pow(0.004, d));
    if (orbit.current) {
      orbit.current.target.lerp(new THREE.Vector3(0, targetY + (earthMode || marsMode ? 1.2 : 0.6), targetZ - (earthMode || marsMode ? 0 : 8.0)), 1 - Math.pow(0.003, d));
