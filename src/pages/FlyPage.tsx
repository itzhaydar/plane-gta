...
export default function FlyPage(){
  ...
  const [phase,setPhase]=useState<FlightPhase>('parked'); const [muted,setMuted]=useState(true); const audioRef=useRef<HTMLAudioElement|null>(null);
  ...
  return <main className="fly-page"><style>{FLY_CSS}</style><Canvas shadows dpr={[1,1.3]} camera={{position:[7.4,4.5,76],fov:43,near:.1,far:1200}} gl={{antialias:true,powerPreference:'high-performance',stencil:false}}><FlightWorld phase={phase} setPhase={setPhase} onTelemetry={setTelemetry}/></Canvas><div className="fly-vignette"/>
    <header className="fly-hud"><div className="flight-brand"><i/>MARSHOUT <b>FLIGHT</b></div><div className="route-card"><span><small>CURRENT LOCATION</small><b>{current}</b></span><em>→</em><span><small>DESTINATION</small><b>{DESTINATION_NAME}</b></span></div><SoundButton muted={muted} onClick={toggleSound}/></header>
    ...
    <aside className="nav-map"><div className="map-title"><span>NAVIGATION</span><b>{telemetry.distance} KM</b></div><div className="map-grid"><div className="route-line"/><span className="city vc">VICE CITY</span><span className="city ls">LOS SANTOS</span><span className="plane-dot" style={{top:`${10+telemetry.progress*78}%`}}>▲</span><i className="radar r1"/><i className="radar r2"/></div><div className="map-bottom"><span>AUTO ROUTE</span><b>{Math.round(telemetry.progress*100)}%</b></div></aside>
